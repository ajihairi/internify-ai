...
the `/internify.work` command: it boots a session, indexes a spec, enforces layered gates,
...
```text
/internify.work .intern/plans/FeatureX
```
...
Inside opencode, the agent drives the same loop through `/internify.work`:

![opencode session running /internify.work](https://raw.githubusercontent.com/ajihairi/internify-ai/main/docs/assets/opencode-session.png)
...
├── adapters/
│   └── opencode/            ← plugin + skill (commands are generated by init)
...
```text
/internify.work .intern/plans/<SpecName>      # opencode
